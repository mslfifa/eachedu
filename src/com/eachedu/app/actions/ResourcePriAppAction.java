...
@Controller("resourcePriAppAction")
@Scope("prototype")
public class ResourcePriAppAction extends BaseAction {
	private static final Logger log = LoggerFactory.getLogger(ResourcePriAppAction.class);
	...
	public void download(){
		...
		try {
			...
			UserVO u = (UserVO)ServletActionContext.getRequest().getSession().getAttribute(ConstUtils.USER_LOGIN);
			
			ResourceInfo r = resourceInfoService.get(riId);
			
			if(ResourceType.COURSEWARE_TYPE.name().equals(r.getResourceType()) ){
				resourceInfoService.checkBuy(u.getId(),u.getAccountType(), riId, r.getResourceType());
			}
